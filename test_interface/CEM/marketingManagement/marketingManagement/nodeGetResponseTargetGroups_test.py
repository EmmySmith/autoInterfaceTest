...
class ICEM_Interface(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.sql = "SELECT id FROM t_workflow_node_define WHERE node_name = '生成目标组' ORDER BY id DESC LIMIT 1;"
        self.dbname = "geek_icem_activity"
        print("----------开始测试----------")


    #获取生成目标组接口
    def test_nodeGetResponseTargetGroups(self):
        '''获取生成目标组接口'''
        self.url = self.host + self.path
        self.nodeDefineId = DB_ICEM_proc(self.dbname).get_vslues(self.sql)
        data = {"nodeDefineId":1999,"type":"TARG_GROUP"}
        print(self.url)
